...
- Default Breeze Full Automatic planning with a quiet low-temperature region, stronger cooling above 70 °C, bounded rise-trend anticipation, 17 safe 5% targets, and an optional Advanced Custom profile.
- An opt-in, default-off setting to resume Breeze Full Automatic after login or wake.
...
- While enabled, Breeze Full Automatic and Advanced Custom keep every calculated target under one continuous Helper watchdog lease instead of handing low temperatures back to Apple. Balanced, Cool, and Max remain explicitly selected fixed presets.
